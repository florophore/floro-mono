...
export default class CommitService {
  ...
  public async writeCommitList(
    ...
    let previousCommitKV = !commitList[commitList.length - 1]?.parent
      ? EMPTY_COMMIT_STATE
      : await this.repoAccessor.readCommitKV(
          repository,
          commitList[commitList.length - 1]?.parent as string
        );
    if (!previousCommitKV) {
    ...
    for (const commitData of commitList) {
      ...
      if (repoHasCommit) {
        previousCommitKV = await this.repoAccessor.readCommitKV(repository, commitData.sha);
        continue;
      }
