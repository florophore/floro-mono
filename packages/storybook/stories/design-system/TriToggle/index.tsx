...
const TriToggle = (props: Props) => {
  ...
  const midBackgrounColor = useMemo(() => {
    if (props.value != props.midOption.value) {
      return theme.background;
    }
    return theme.colors.toggleColor;
  ...
  const onClickMid = useCallback(() => {
    if (props?.isDisabled) {
      return;
    }
    props.onChange(props.midOption.value);
  }, [props.midOption, props.onChange, props?.isDisabled]);
  ...
  return (
    <Container style={{borderColor}}>
      ...
      <Option
        onClick={onClickMid}
        style={{
          backgroundColor: midBackgrounColor,
          borderLeft: `1px solid ${borderColor}`,
          borderRight: `1px solid ${borderColor}`,
        }}
      >
        <OptionText style={{ color: midColor }}>
          {props.midOption.label}
        </OptionText>
      </Option>
      <Option
...
export default React.memo(TriToggle);
