...
function CLIDocs() {
  ...
  // change this
  const docsMetaTitle = usePlainText("meta_tags.cli_docs");
  ...
  const cliDocsTitle = usePlainText("doc_titles.cli_docs_page_title");
  ...
  const article = useRichText(
    "dev_docs.cli_docs_floro",
    {
      ...
      floroModule: `floro module`,
      floroModuleHelp: `$ floro module --help`,
      floroModuleHelpOutput: MODULE_HELP_OUTPUT,
      floroModuleJS: (
        <CodeOut style={{fontSize: '1.2rem', overflowX: 'scroll'}}>
      ...
      command: function (
        ...
        return <Command style={{maxWidth: 300}}>{content}</Command>;
      },
      commandReponse: function (
        content: ReactElement<any, string | JSXElementConstructor<any>>
      ): ReactElement<any, string | JSXElementConstructor<any>> {
        return <CommandOut style={{maxWidth: 600}}>{content}</CommandOut>;
      },
    },
    rtRenderers
  );
