...
import {
  ...
  useUpdateRepoAnnouncementReplyMutation,
...
const CommentDisplay = (props: Props) => {
  ...
  const [edit, editMutation] = useUpdateRepoAnnouncementReplyMutation();

  const onFinishEdit = useCallback(() => {
    if (isDisabled) {
      return;
    }
    if (!props?.repoAnnouncement?.repository?.id || !props?.repoAnnouncement?.id || !props?.reply?.id) {
      return;
    }
    edit({
      variables: {
  ...
  }, [isDisabled, text, props?.repoAnnouncement?.repository, props?.repoAnnouncement, props?.reply]);

  useEffect(() => {
    if (editMutation?.data?.updateRepoAnnouncementReplyComment?.__typename == "UpdateRepoAnnouncementReplySuccess") {
      props?.onHide();
    }
  }, [editMutation?.data?.updateRepoAnnouncementReplyComment?.__typename, props?.onHide])
