...
const MergeBranchModal = (props: Props) => {
  ...
  const [mergeBranch, mergeBranchMutation] = useMergeBranchMutation()
  const title = useMemo(() => {
    return (
      <HeaderContainer>
        <HeaderTitle>{"merge branch"}</HeaderTitle>
      </HeaderContainer>
    );
  }, []);
  ...
  useEffect(() => {
    if (mergeBranchMutation?.data?.mergeBranch?.__typename == "MergeBranchSuccess") {
      props?.onSuccess();
    }
  }, [props?.onDismiss, mergeBranchMutation?.data?.mergeBranch?.__typename])
    ...
    >
      <ContentContainer>
        <TopContentContainer>
          ...
          <PromptText>
            {`Are you sure you want to delete the branch "${props?.branch?.name}" into "${props?.baseBranch?.name}"?`}
          </PromptText>
        </TopContentContainer>
        <BottomContentContainer>
          ...
          <Button
            ...
            size={"medium"}
            onClick={onDelete}
            isLoading={mergeBranchMutation.loading}
          />
        </BottomContentContainer>
      </ContentContainer>
    </RootModal>
...
export default React.memo(MergeBranchModal);