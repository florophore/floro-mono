...
const CanCreateMergeRequestsUsersModal = (props: Props) => {
  ...
  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanCreateMergeRequestsUsersMutation();
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.protectedBranchRule?.canCreateMergeRequestsUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanCreateMergeRequestsUsers?.__typename == "ProtectedBranchSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanCreateMergeRequestsUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"create merge request users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable merge request create access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can create merge requests"}</UserLabelText>
