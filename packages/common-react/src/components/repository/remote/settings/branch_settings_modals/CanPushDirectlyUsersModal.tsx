...
import {
  ...
  useUpdateCanPushDirectlyUsersMutation,
...
const CanPushDirectlyUsersModal = (props: Props) => {
  const searchRef = useRef<HTMLInputElement>(null);
  const [userInput, setUserInput] = useState("");

  const [updateUsers, updateUsersRequest] = useUpdateCanPushDirectlyUsersMutation();
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.protectedBranchRule?.canPushDirectlyUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  useEffect(() => {
    if (updateUsersRequest?.data?.updateCanPushDirectlyUsers?.__typename == "ProtectedBranchSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateCanPushDirectlyUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"can push directly users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable direct push access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can push directly"}</UserLabelText>
...
export default React.memo(CanPushDirectlyUsersModal);
