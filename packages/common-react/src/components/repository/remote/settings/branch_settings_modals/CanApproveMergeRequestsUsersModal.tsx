...
import {
  ...
  useUpdateAnyoneCanApproveMergeRequestsUsersMutation,
...
const CanCreateMergeRequestsUsersModal = (props: Props) => {
  const theme = useTheme();
  const searchRef = useRef<HTMLInputElement>(null);
  const [userInput, setUserInput] = useState("");

  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanApproveMergeRequestsUsersMutation();
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.protectedBranchRule?.canApproveMergeRequestsUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanApproveMergeRequestsUsers?.__typename == "ProtectedBranchSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanApproveMergeRequestsUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"approve merge request users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable merge request approval access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can approve merge requests"}</UserLabelText>
...
export default React.memo(CanCreateMergeRequestsUsersModal);
