...
const WithApprovalCanMergeRolesModal = (props: Props) => {
  const [assignedRoles, setAssignedRoles] = useState<Set<string>>(new Set([]));

  useEffect(() => {
    if (props.show) {
      setAssignedRoles(
        new Set(props?.repository?.protectedBranchRule?.withApprovalCanMergeRoles?.map((v) => v?.id as string) ?? [])
      );
    }
  ...
  useEffect(() => {
    if (
      updateRolesResult?.data?.updateAnyoneWithApprovalCanMergeRoles?.__typename ==
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"can merge with approval roles"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentContainer>
...
export default React.memo(WithApprovalCanMergeRolesModal);
