...
export interface Props {
  ...
  selectedDefaultBranch: Branch|undefined;
}

const ConfirmDefaultBranchChangeModal = (props: Props) => {
  ...
  const [changeDefaultBranch, changeDefaultBranchRequest] = useChangeDefaultBranchMutation();
  ...
  const onChange = useCallback(() => {
    if (!props.repository.id || !props?.selectedDefaultBranch?.id) {
      return;
    }
    changeDefaultBranch({
      variables: {
        repositoryId: props?.repository?.id,
        branchId: props?.selectedDefaultBranch?.id,
      },
    });
  }, [props.repository.id, props?.selectedDefaultBranch?.id]);

  useEffect(() => {
    if (changeDefaultBranchRequest?.data?.changeDefaultBranch?.__typename == "ChangeDefaultBranchSuccess") {
      props?.onSuccess();
    }
  }, [props?.onSuccess, changeDefaultBranchRequest?.data?.changeDefaultBranch])
    ...
    >
      <ContentContainer>
        <TopContentContainer>
          ...
          <PromptText>
            {`Are you sure you want to change the default branch to ${props.selectedDefaultBranch?.name}?`}
          </PromptText>
          <PromptText>
            {"This will affect all users of the repository and may impact production systems."}
          </PromptText>
        </TopContentContainer>
        <BottomContentContainer>
          ...
          <Button
            ...
            size={"medium"}
            onClick={onChange}
            isLoading={changeDefaultBranchRequest.loading}
          />
        </BottomContentContainer>
      </ContentContainer>
    </RootModal>
...
export default React.memo(ConfirmDefaultBranchChangeModal);