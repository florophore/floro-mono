...
import {
  ...
  useUpdateAnyoneCanWriteAnnouncementsUsersMutation,
...
const AddWriteAnnouncementsAccessUsersModal = (props: Props) => {
  const searchRef = useRef<HTMLInputElement>(null);
  const [userInput, setUserInput] = useState("");

  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanWriteAnnouncementsUsersMutation();
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.canWriteAnnouncementsUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  const userResults = useMemo((): Array<User> => {
    ...
    if (
      searchUsersResult?.data?.searchUsersForRepoPushAccess?.__typename ==
      "SearchUsersForSettingSuccess"
    ) {
      return (searchUsersResult?.data?.searchUsersForRepoPushAccess
        ?.users ?? []) as Array<User>;
    }
    return [];
  ...
  }, [users, props.repository?.id])

  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanWriteAnnouncementsUsers?.__typename == "RepoSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanWriteAnnouncementsUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo announcement users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable announcement publish access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can write announcements for repo"}</UserLabelText>
...
export default React.memo(AddWriteAnnouncementsAccessUsersModal);
