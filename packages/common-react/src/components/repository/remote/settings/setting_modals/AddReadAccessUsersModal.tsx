...
import {
  User,
  Repository,
  useSearchUsersForRepoReadLazyQuery,
  useUpdateAnyoneCanReadUsersMutation,
...
const AddReadAccessUsersModal = (props: Props) => {
  ...
  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanReadUsersMutation();
  ...
  const hasEnabledRole = useMemo(() => {
    const enabledRoleIds = new Set(
      props?.repository?.canReadRoles?.map((r) => r?.id) ?? []
    );
    ...
    return false;
  }, [props?.repository?.canReadRoles, memberRoleIds]);
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.canReadUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  const userResults = useMemo((): Array<User> => {
    ...
    if (
      searchUsersResult?.data?.searchUsersForRepoReadAccess?.__typename ==
      "SearchUsersForSettingSuccess"
    ) {
      return (searchUsersResult?.data?.searchUsersForRepoReadAccess
        ?.users ?? []) as Array<User>;
    }
    return [];
  ...
  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanReadUsers?.__typename == "RepoSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanReadUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo read access users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable read access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can read repo"}</UserLabelText>
...
export default React.memo(AddReadAccessUsersModal);
