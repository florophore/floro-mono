...
const AddReadAccessRolesModal = (props: Props) => {
  ...
  useEffect(() => {
    if (props.show) {
      setAssignedRoles(
        new Set(props?.repository?.canPushBranchesRoles?.map((v) => v?.id as string) ?? [])
      );
    }
  ...
  useEffect(() => {
    if (
      updateRolesResult?.data?.updateAnyoneCanPushBranchesRoles?.__typename ==
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo push roles"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentContainer>
