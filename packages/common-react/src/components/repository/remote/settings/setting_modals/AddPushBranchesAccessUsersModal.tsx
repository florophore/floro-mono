...
const AddPushBranchesAccessUsersModal = (props: Props) => {
  const theme = useTheme();
  const searchRef = useRef<HTMLInputElement>(null);
  const [userInput, setUserInput] = useState("");

  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanPushBranchesUsersMutation();
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.canPushBranchesUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanPushBranchesUsers?.__typename == "RepoSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanPushBranchesUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo push access users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable push access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can push to repo"}</UserLabelText>
...
export default React.memo(AddPushBranchesAccessUsersModal);
