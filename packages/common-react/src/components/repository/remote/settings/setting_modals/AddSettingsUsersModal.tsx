...
import {
  User,
  Repository,
  useSearchUsersForRepoSettingLazyQuery,
  useUpdateAnyoneCanChangeSettingsUsersMutation,
...
const AddSettingsUsersModal = (props: Props) => {
  const theme = useTheme();
  const searchRef = useRef<HTMLInputElement>(null);
  const [userInput, setUserInput] = useState("");
  const { currentUser } = useSession();

  const [updateUsers, updateUsersRequest] = useUpdateAnyoneCanChangeSettingsUsersMutation();
  ...
  const hasEnabledRole = useMemo(() => {
    const enabledRoleIds = new Set(
      props?.repository?.canChangeSettingsRoles?.map((r) => r?.id) ?? []
    );
    for (const memberRoleId of memberRoleIds) {
      if (enabledRoleIds.has(memberRoleId)) {
        return true;
      }
    }
    return false;
  }, [props?.repository?.canChangeSettingsRoles, memberRoleIds]);
  ...
  useEffect(() => {
    if (props.show) {
      ...
      const nextUsers = (props?.repository?.canChangeSettingsUsers ??
        []) as Array<User>;
      setUsers(nextUsers);
  ...
  const userResults = useMemo((): Array<User> => {
    ...
    if (
      searchUsersResult?.data?.searchUsersForRepoSettingsAccess?.__typename ==
      "SearchUsersForSettingSuccess"
    ) {
      return (searchUsersResult?.data?.searchUsersForRepoSettingsAccess
        ?.users ?? []) as Array<User>;
    }
    return [];
  ...
  const disableUpdate = useMemo(() => {
    if (isAdmin || hasEnabledRole) {
      return false;
    }
    const userIds = new Set(users?.map((u) => u.id));
    return !userIds.has(currentUser?.id);
  }, [isAdmin, hasEnabledRole, currentUser?.id, users]);

  const onUpdate = useCallback(() => {
    if (disableUpdate || !props?.repository?.id) {
      return;
    }
  ...
  }, [users, disableUpdate, props.repository?.id])

  useEffect(() => {
    if (updateUsersRequest?.data?.updateAnyoneCanChangeSettingsUsers?.__typename == "RepoSettingChangeSuccess") {
      props?.onDismissModal();
    }
  }, [updateUsersRequest?.data?.updateAnyoneCanChangeSettingsUsers, props?.onDismissModal])
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo setting users"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentWrapper>
        <TopWrapper>
          ...
          <div style={{ width: 468, display: "block", textAlign: "left" }}>
            <InstructionsText>
              {
                "Search for members to enable settings access by @username, or first name and last name."
              }
            </InstructionsText>
          </div>
        </TopWrapper>
        <BottomWrapper>
          <UserContainer>
            ...
            <UserLabelContainer>
              ...
              <UserLabelText>{"users who can change settings"}</UserLabelText>
...
export default React.memo(AddSettingsUsersModal);
