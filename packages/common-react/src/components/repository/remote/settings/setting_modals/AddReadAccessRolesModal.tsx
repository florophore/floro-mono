...
const AddReadAccessRolesModal = (props: Props) => {
  ...
  const enabledUserIds = useMemo((): Set<string> => {
    return new Set(
      props?.repository?.canReadUsers?.map((v) => v?.id as string) ??
      []
    );
  }, [props?.repository?.canReadUsers]);
  ...
  useEffect(() => {
    if (props.show) {
      setAssignedRoles(
        new Set(props?.repository?.canReadRoles?.map((v) => v?.id as string) ?? [])
      );
    }
  ...
  useEffect(() => {
    if (
      updateRolesResult?.data?.updateAnyoneCanReadRoles?.__typename ==
  ...
  return (
    <RootLongModal
      ...
      headerChildren={
        <HeaderWrapper>
          <FloroHeaderTitle>{"edit repo read roles"}</FloroHeaderTitle>
        </HeaderWrapper>
      }
    >
      <ContentContainer>
...
export default React.memo(AddReadAccessRolesModal);
