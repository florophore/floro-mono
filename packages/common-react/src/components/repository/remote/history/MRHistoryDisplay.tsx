...
const MRHistoryDisplay = (props: Props) => {
  ...
  const commits = useMemo(() => {
    return props?.repository?.mergeRequest?.commits ?? []
  ...
  const commitsLink = useMemo(() => {
    return `${linkBase}/mergerequests/${props?.repository?.mergeRequest?.id}?from=remote&plugin=${props.plugin ?? "home"}&review_page=commits`;
  ...
  if (props?.repository?.mergeRequest?.commitsCount == 0) {
    return (
        <Container>
        <NoCommitsContainer>
  ...
  return (
    <Container ref={container}>
      ...
      {((showNewer || showOlder) && (props?.repository?.mergeRequest?.commitsCount ?? 0) > PAGINATION_SIZE) && (
        <div
          style={{
            marginTop: 24,
            marginBottom: 48,
...
export default React.memo(MRHistoryDisplay);