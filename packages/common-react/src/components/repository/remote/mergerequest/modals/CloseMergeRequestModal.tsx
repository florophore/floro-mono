...
const CloseMergeRequestModal = (props: Props) => {
  ...
  const [closeMergeRequest, closeMergeRequestMutation] = useCloseMergeRequestMutation();

  const title = useMemo(() => {
    return (
      <HeaderContainer>
        <HeaderTitle>{"close merge request"}</HeaderTitle>
      </HeaderContainer>
    );
  }, []);
  ...
  useEffect(() => {
    if (closeMergeRequestMutation?.data?.closeMergeRequest?.__typename == "CloseMergeRequestSuccess") {
      props?.onDismiss();
    }
  }, [props?.onDismiss, closeMergeRequestMutation?.data?.closeMergeRequest])
    ...
    >
      <ContentContainer>
        <TopContentContainer>
          ...
          {props?.mergeRequest?.approvalStatus != "pending" && (
            <PromptText>
              {
                "You will have to have your merge request re-reviewed if you decide to open a new merge request against this branch."
              }
            </PromptText>
          )}
        </TopContentContainer>
        <BottomContentContainer>
          ...
          <Button
            ...
            size={"medium"}
            onClick={onChange}
            isLoading={closeMergeRequestMutation.loading}
          />
        </BottomContentContainer>
      </ContentContainer>
    </RootModal>
