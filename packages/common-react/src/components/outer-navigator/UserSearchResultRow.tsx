...
const UserSearchResultRow = (props: Props) => {
  ...
  const photo = useOfflineIcon(
    props?.user?.profilePhoto?.thumbnailUrl ?? undefined
  );
    ...
    <Row
      style={{
        background: props.isSelected
          ? theme.colors.highlightedOptionBackgroundColor
          : theme.background,
      }}
    >
      <LeftSide>
        <div style={{ marginRight: 24 }}>
          <UserProfilePhoto size={56} user={props.user} offlinePhoto={photo} />
        </div>
      </LeftSide>
      <CenterInfo>
        <DisplayTitle
