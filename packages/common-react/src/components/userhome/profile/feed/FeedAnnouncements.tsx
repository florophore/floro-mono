...
const FeedAnnouncements = (props: Props) => {
  const [repoAnnouncements, setRepoAnnouncements] = useState<
    RepoAnnouncement[]
  >([]);
  const [lastId, setLastId] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState<boolean>(false);
  const [fetch, fetchAnnouncementsQuery] = useFetchFeedLazyQuery();
  ...
  useEffect(() => {
    if (
      fetchAnnouncementsQuery.data?.fetchFeed?.__typename ==
      "FetchRepoAnnouncementsResult"
    ) {
      const ids = new Set(
        fetchAnnouncementsQuery.data?.fetchFeed?.announcements?.map(
          (a) => a?.id
        ) ?? []
      );
      setRepoAnnouncements(
        [
          ...
          ...((fetchAnnouncementsQuery.data?.fetchFeed.announcements ??
      ...
      if (
        ...
        fetchAnnouncementsQuery.data?.fetchFeed.announcements?.[0]?.id
      ) {
        setHasMore(fetchAnnouncementsQuery.data?.fetchFeed.hasMore ?? false);
        setLastId(fetchAnnouncementsQuery.data?.fetchFeed.lastId ?? null);
      }
    }
      ...
      <InsufficientPermssionsContainer>
        <InsufficientPermssionsTextWrapper>
          <NoAnnouncementsTitle style={{textAlign: 'center', fontSize: '1.7rem'}}>
            {
              "No announcements to display. Subscribe to repositories to follow their announcements."
            }
...
export default React.memo(FeedAnnouncements);
