...
import {
  ...
  useFetchUserRepoAnnouncementsLazyQuery,
...
const UserAnnouncements = (props: Props) => {
  const [repoAnnouncements, setRepoAnnouncements] = useState<
    RepoAnnouncement[]
  >([]);
  const [lastId, setLastId] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState<boolean>(false);
  const [fetch, fetchAnnouncementsQuery] = useFetchUserRepoAnnouncementsLazyQuery();
  ...
  useEffect(() => {
    if (
      fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements?.__typename ==
      "FetchRepoAnnouncementsResult"
    ) {
      const ids = new Set(
        fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements?.announcements?.map(
          (a) => a?.id
        ) ?? []
      );
      setRepoAnnouncements(
        [
          ...
          ...((fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements.announcements ??
      ...
      if (
        ...
        fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements.announcements?.[0]?.id
      ) {
        setHasMore(fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements.hasMore ?? false);
        setLastId(fetchAnnouncementsQuery.data?.fetchUserRepoAnnouncements.lastId ?? null);
      }
    }
      ...
      <InsufficientPermssionsContainer>
        <InsufficientPermssionsTextWrapper>
          <NoAnnouncementsTitle style={{textAlign: 'center', fontSize: '1.7rem'}}>
            {
              `${firstName} hasn't posted any announcements to their public repositories yet.`
            }
...
export default React.memo(UserAnnouncements);
