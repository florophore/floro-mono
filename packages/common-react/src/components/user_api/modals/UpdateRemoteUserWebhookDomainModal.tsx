...
import { WebhookKey, useUpdateUserWebhookKeyMutation } from "@floro/graphql-schemas/src/generated/main-client-graphql";
...
const UpdateRemoteUserWebhookDomainModal = (props: Props) => {
  ...
  const [updateKey, updateKeyMutation] = useUpdateUserWebhookKeyMutation();
  ...
  const onUpdateKey = useCallback(() => {
    if (!isValid || !props?.webhookKey?.id) {
      return;
    }
    updateKey({
      variables: {
  ...
  useEffect(() => {
    if (updateKeyMutation.data?.updateUserWebhookKey?.__typename == "UserWebhookKeySuccess") {
      props.onDismissModal();
    }
  }, [updateKeyMutation.data?.updateUserWebhookKey?.__typename]);
...
export default React.memo(UpdateRemoteUserWebhookDomainModal);
