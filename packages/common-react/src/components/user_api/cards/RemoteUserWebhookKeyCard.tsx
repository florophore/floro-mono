...
import { WebhookKey, useDeleteUserApiKeyMutation, useDeleteUserWebhookKeyMutation, useRegenerateUserApiKeyMutation, useRegenerateUserWebhookKeyMutation, useUpdateUserWebhookKeyEnabledMutation, useVerifyUserWebhookKeyMutation } from "@floro/graphql-schemas/src/generated/main-client-graphql";
import UpdateRemoteUserWebhookDomainModal from "../modals/UpdateRemoteUserWebhookDomainModal";
...
const RemoteWebhookKeyCard = (props: Props): React.ReactElement => {
  ...
  const [regenerateKey, regenerateKeyMutation] = useRegenerateUserWebhookKeyMutation();
  const [deleteKey, deleteKeyMutation] = useDeleteUserWebhookKeyMutation();
  const [verifyKey, verifyMutation] = useVerifyUserWebhookKeyMutation();
  const [changeEnablement, changeEnablementMutation] = useUpdateUserWebhookKeyEnabledMutation();

  const onToggleEnablement = useCallback(() => {
    if (changeEnablementMutation.loading || !props?.webhookKey?.id) {
      return;
    }
    changeEnablement({
      variables: {
  ...
  }, [props.webhookKey.isEnabled, changeEnablementMutation.loading])

  const onRegenKey = useCallback(() => {
    if (!props.webhookKey.id) {
      return;
    }
    regenerateKey({
      variables: {
  ...
  useEffect(() => {
    if (regenerateKeyMutation.data?.regenerateUserWebhookKey?.__typename == "UserWebhookKeySuccess") {
      regenerateKeyMutation.reset();
      setShowConfirmRegenerate(false);
    }
  }, [regenerateKeyMutation.data?.regenerateUserWebhookKey?.__typename])

  useEffect(() => {
    if (deleteKeyMutation.data?.deleteUserWebhookKey?.__typename == "UserWebhookKeySuccess") {
      deleteKeyMutation.reset();
      setShowConfirmDelete(false);
    }
  }, [deleteKeyMutation.data?.deleteUserWebhookKey?.__typename])

  useEffect(() => {
    if (verifyMutation.data?.verifyUserWebhookKey?.__typename == "UserWebhookKeySuccess") {
      verifyMutation.reset();
      setShowVerifcationSucceded(true);
    }
  }, [verifyMutation.data?.verifyUserWebhookKey?.__typename])

  useEffect(() => {
    if (verifyMutation.data?.verifyUserWebhookKey?.__typename == "UserWebhookKeyError") {
      verifyMutation.reset();
...
export default React.memo(RemoteWebhookKeyCard);
