...
import {
  ...
  useRegenerateUserApiKeyMutation,
  useUpdateUserApiKeyEnabledMutation,
...
const LocalApiKeyCard = (props: Props): React.ReactElement => {
  ...
  const [regenerateKey, regenerateKeyMutation] = useRegenerateUserApiKeyMutation();
  const [deleteKey, deleteKeyMutation] = useDeleteUserApiKeyMutation();
  const [changeEnablement, changeEnablementMutation] = useUpdateUserApiKeyEnabledMutation();
  ...
  const onToggleEnablement =  useCallback(() => {
    if (changeEnablementMutation.loading || !props?.apiKey?.id) {
      return;
    }
    changeEnablement({
      variables: {
  ...
  }, [props.apiKey.isEnabled, changeEnablementMutation.loading])
  ...
  useEffect(() => {
    if (regenerateKeyMutation.data?.regenerateUserApiKey?.__typename == "UserApiKeySuccess") {
      regenerateKeyMutation.reset();
      setShowConfirmRegenerate(false);
    }
  }, [regenerateKeyMutation.data?.regenerateUserApiKey?.__typename])

  useEffect(() => {
    if (deleteKeyMutation.data?.deleteUserApiKey?.__typename == "UserApiKeySuccess") {
      deleteKeyMutation.reset();
      setShowConfirmDelete(false);
    }
  }, [deleteKeyMutation.data?.deleteUserApiKey?.__typename])
