...
const ConfirmRegenerateModal = (props: Props) => {
  ...
  const title = useMemo(() => {
    return (
      <HeaderContainer>
        <HeaderTitle>{"regenerate secret"}</HeaderTitle>
      </HeaderContainer>
    );
  }, []);
    ...
    >
      <ContentContainer>
        <TopContentContainer>
          ...
          <PromptText>
            {`Are you sure you want to regenerate the secret for "${props?.keyName}"?`}
          </PromptText>
          <PromptText>
            {
              "This will invalidate the previous secret and may cause issues if the key is being used in a production context."
            }
          </PromptText>
        </TopContentContainer>
...
export default React.memo(ConfirmRegenerateModal);