...
const Layout = () => {
  ...
  const [updateIconRef, setUpdateIconRef] =
    useState<PointerTypes["$(icons).iconGroups.id<?>.icons.id<?>"]>();
  const [updateIcon, setUpdateIcon] =
    useState<SchemaTypes["$(icons).iconGroups.id<?>.icons.id<?>"]>();

  const onShowAddSVG = useCallback(
    (svgFileRef: FileRef, svgFileName: string) => {
