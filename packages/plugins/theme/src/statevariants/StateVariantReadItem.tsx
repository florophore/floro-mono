...
const stateVairiantItemVariants = {
  hidden: { opacity: 0 },
...
const StateVariantReadItem = (props: ShadeItemProps) => {
  const theme = useTheme();
  const shadeQuery = useQueryRef("$(theme).stateVariants.id<?>", props.stateVariant.id);
  ...
  return (
    <motion.li
      variants={stateVairiantItemVariants}
      initial={"hidden"}
      animate={"visible"}
      exit={"hidden"}
    ...
    >
      <ShadeContainer>
        ...
        <ShadeLabel style={{ color }}>{isInvalid ? props?.stateVariant?.id : props?.stateVariant.name}</ShadeLabel>
      </ShadeContainer>
    </motion.li>
  );
};

export default React.memo(StateVariantReadItem);