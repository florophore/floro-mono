...
const StyledContentReOrderRow = (props: Props) => {
  ...
  return (
    <Reorder.Item
      ...
      value={props.styledContent}
      variants={colorPaletteItemVariants}
      initial={"hidden"}
      animate={"visible"}
      exit={"hidden"}
      layoutId={props.styledContent.name}
      custom={(props.index + 1) * 0.005}
      whileHover={{ scale: 1 }}
      whileDrag={{ scale: 1.02 }}
      key={props.styledContent.name}
...
export default React.memo(StyledContentReOrderRow);
