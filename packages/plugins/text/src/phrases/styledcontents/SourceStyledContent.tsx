...
interface Props {
  ...
  targetStyledContentLocaleRule: SchemaTypes["$(text).phraseGroups.id<?>.phrases.id<?>.styledContents.name<?>.localeRules.id<?>"];
}

const SourceStyledContent = (props: Props) => {
  ...
  }, [props.targetStyledContentLocaleRule.displayValue?.sourceAtRevision?.plainText]);
        ...
        {enabledMentionedTerms?.length > 0 && (
          <TermList
            ...
            title={(
              <>
                <span style={{ color: theme.colors.contrastText }}>
                  {`Mentioned Terms in Source Content Value of `}
                </span>
                <span
                  style={{
                    fontSize: "1.4rem",
...
export default React.memo(SourceStyledContent);
