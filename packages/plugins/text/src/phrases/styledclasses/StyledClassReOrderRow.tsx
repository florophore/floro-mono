...
const StyleClassReOrderRow = (props: Props) => {
  ...
  return (
    <Reorder.Item
      ...
      value={props.styleClass}
      variants={colorPaletteItemVariants}
      initial={"hidden"}
      animate={"visible"}
      exit={"hidden"}
      layoutId={props.styleClass.name}
      custom={(props.index + 1) * 0.005}
      whileHover={{ scale: 1 }}
      whileDrag={{ scale: 1.02 }}
      key={props.styleClass.name}
...
export default React.memo(StyleClassReOrderRow);
