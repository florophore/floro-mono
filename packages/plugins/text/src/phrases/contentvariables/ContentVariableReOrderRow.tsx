...
const ContentVariableReOrderRow = (props: Props) => {
  ...
  return (
    <Reorder.Item
      ...
      value={props.contentVariable}
      variants={colorPaletteItemVariants}
      initial={"hidden"}
      animate={"visible"}
      exit={"hidden"}
      layoutId={props.contentVariable.name}
      custom={(props.index + 1) * 0.005}
      whileHover={{ scale: 1 }}
      whileDrag={{ scale: 1.02 }}
      key={props.contentVariable.name}
...
export default React.memo(ContentVariableReOrderRow);
