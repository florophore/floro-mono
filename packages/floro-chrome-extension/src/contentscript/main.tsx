const setupListeners = () => {
  window.addEventListener("floro:module:register", (event: CustomEvent) => {
    chrome.runtime.sendMessage({
      ...
      metaFile: event?.detail,
    });
  });

  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type == "update:state") {
      const updateEvent = new CustomEvent("floro:update-state", {
        detail: msg.update,
      });
      window.dispatchEvent(updateEvent);
    }
    if (msg.type == "toggle:edit_mode") {
      const updateEvent = new CustomEvent("floro:toggle-edit-mode", {
        detail: msg.isEditMode,
      });
      window.dispatchEvent(updateEvent);
    }
    if (msg.type == "toggle:debug_mode") {
      const updateEvent = new CustomEvent("floro:toggle-debug-mode", {
        detail: msg.isDebugMode,
      });
      window.dispatchEvent(updateEvent);
    }
    return false;
  ...
  chrome.runtime.sendMessage({ type: "get-tab-state" }, (msg) => {
    if (msg.type == "get-tab-state-response") {
      const updateEditEvent = new CustomEvent("floro:toggle-edit-mode", {
        detail: msg.tabState.isEditMode,
      });
      window.dispatchEvent(updateEditEvent);
      const updateDebugEvent = new CustomEvent("floro:toggle-debug-mode", {
        detail: msg.tabState.isDebugMode,
      });
