...
export default class UsersContext extends BaseContext {
  ...
  public async searchUsers(query: string, limit = 5): Promise<User[]> {
    try {
      ...
      return await qb
        .leftJoinAndSelect("user.profilePhoto", "photo")
        .where(
          `(user.first_name || ' '  || user.last_name ILIKE :query || '%') OR (user.last_name ILIKE :query || '%')`
        )
  ...
  public async searchUsersExcludingIds(
    query: string,
    excludingIds: string[],
    limit = 5
  ): Promise<User[]> {
    try {
      ...
      return await qb
        .leftJoinAndSelect("user.profilePhoto", "photo")
        .where(
          `((user.first_name || ' '  || user.last_name ILIKE :query || '%') OR (user.last_name ILIKE :query || '%')) AND user.id <> ALL(:ids)`
        )
        .setParameter("query", query.trim().toLowerCase())
        .setParameter("ids", excludingIds)
        .limit(limit)
  ...
  public async searchUsersIncludingIdsAndExcludingIds(
    query: string,
    includingIds: string[],
    excludingIds: string[],
    limit = 5
    ...
    try {
      ...
      if (query.startsWith("@")) {
        ...
        return await qb
          .leftJoinAndSelect("user.profilePhoto", "photo")
          .where(
            "(user.username ILIKE :query || '%') AND user.id <> ALL(:excluded_ids) AND user.id IN(:...included_ids)"
          )
          .setParameter("query", usernameQuery.trim().toLowerCase())
      ...
      return await qb
        .leftJoinAndSelect("user.profilePhoto", "photo")
        .where(
          `((user.first_name || ' '  || user.last_name ILIKE :query || '%') OR (user.last_name ILIKE :query || '%')) AND user.id <> ALL(:excluded_ids) AND user.id IN(:...included_ids)`
        )
        .setParameter("query", query.trim().toLowerCase())
        .setParameter("excluded_ids", excludingIds)
        .setParameter("included_ids", includingIds)
